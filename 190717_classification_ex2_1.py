...
if __name__ == "__main__":
    data = np.loadtxt("./data/data-02.csv", delimiter=",", dtype=np.float32)
    x_data = data[:, 0:-1]
    t_data = data[:, [-1]]

    test = SimpleLogisticRegression(x_data, t_data, 1e-3, 100000)
    test.train(debug=True, interval=10000)
